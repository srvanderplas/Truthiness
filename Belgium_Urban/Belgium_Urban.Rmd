...
library(geojsonio)
belgium_italy <- geojson_read("../Data/belgiumItaly.geo.json", what = "sp")
...
if (!file.exists("../Data/belgiummaptiles.Rdata")) {
  belgiummaptiles <- get_stamenmap(belgiumbox, zoom = 8) 
  save(belgiummaptiles, file = "../Data/belgiummaptiles.Rdata")
} else {
  load("../Data/belgiummaptiles.Rdata")
}
...
if (!file.exists("../Data/italymaptiles.Rdata")) {
  italymaptiles <- get_stamenmap(italybox, zoom = 6)
  save(italymaptiles, file = "../Data/italymaptiles.Rdata")
} else {
  load("../Data/italymaptiles.Rdata")
}
italymaptiles %>% ggmap() + 
...
labs <- location %>%
  filter(name %in% c("France", "Germany", "United Kingdom", "Greece", "Italy", "Spain", "Belgium"))  %>%
  mutate(name = str_replace_all(name, c("United Kingdom" = "UK", "Czechia"= "Czech Republic"))) 
...
tmp %>%
  ...
  coord_quickmap(xlim = c(lims$long_min, lims$long_max - 15), ylim = c(lims$lat_min, lims$lat_max+5), expand = T) + 
