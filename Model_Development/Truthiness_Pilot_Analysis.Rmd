...
$$ \text{logit}(\text{True Evaluation}) \propto \alpha_1 + \beta_1 1_{subj. related}$$ 
...
# with odbc.
file.copy(from = file.path(here::here("Data"), "Driver={SQLite3};dbname=truthinessStudy.db"),
          to = file.path("./"), overwrite = T)
...
model <- glmer(answer ~ (1|factCode) + factor(trialType) + correctAnswer + (1|userID), 
               data = vanilla_trials, 
               family = "binomial")
summary(model)
